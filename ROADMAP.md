...
## Pendências abertas
- [ ] **Escolher a versão do modpack ao criar** — hoje o painel pega a última versão automaticamente. Quando o pack tem várias (versões de jogo diferentes ou releases do pack), mostrar um **seletor de versão** no card/confirmação antes de criar (o backend `modpackVersions()` já lista tudo; `createFromModpack` já aceita `versionId` — falta a UI). Ex.: Essential Optimized existe em 26.2, 26.1.2, etc.
- [ ] **Botão "desligar tudo" (parada de emergência)** — parar de uma vez **qualquer** instância `minecraft@*` que esteja rodando, pra casos de memory leak/thrash (como o do notebook de 4 GB). Um botão no painel + endpoint que faz `systemctl stop` em todas as ativas (e, se preciso, kill mais agressivo com timeout curto). Útil quando a máquina está engasgada.
- [ ] **Fidelidade do status no Painel inicial** — o estado "ligado/desligado" não reflete a realidade (diz que está ligado mas não parece estar de fato no ar). Hoje deve usar só `systemctl is-active`, que fica "active" antes do servidor aceitar conexões (ou mesmo travado em swap). Melhorar pra distinguir **iniciando vs no ar de verdade**: checar `Done` no log / porta aceitando conexão / RCON respondendo, e refletir isso no hero e no dot de status.
