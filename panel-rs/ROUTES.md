...
| GET | `/api/content/download-all` | `?server` | `application/zip` (stored, sem compressão), `Content-Length`, `attachment; filename="<id>-mods.zip"`; `400` (Paper/vanilla); `404` (sem mods) | zip = `LEIA-ME.txt` + `.jar` ativos + `.jar.disabled` listados em `strippedMods` (voltam a `.jar`); montado num temporário apagado depois; Rust e Node geram o mesmo arquivo byte a byte; audit `mods-baixar-zip` | **FEITA** |
| POST | `/api/content/upload` | `?server&name`; corpo cru (`application/octet-stream`, até 256 MB só com sessão válida) | `{ok, file, folder:"mods"\|"plugins", replaced}`; `400 {error}` (não é `.jar` / não é zip) | grava em `mods/` ou `plugins/` conforme o loader via `.part` + rename, apaga o `.disabled` de mesmo nome; audit `mod-enviar` | **FEITA** |
...
| POST | `/api/servers/stop-all` | `{force?}` | `{ok, stopped:[ids], errors:[{id,error}], force}`; `500` se algum falhou | para toda instância `active`/`activating`/`deactivating`: `stop` pelo RCON quando responde, senão `systemctl stop --no-block`; `force` = `stop --no-block` + `kill --signal=SIGKILL`; audit `desligar-tudo[-forcado]` | **SÓ RUST** |
| GET | `/api/system/update` | — | `{supported:false, reason}` (Docker/rootless/sem updater) ou `{supported:true, running, current, latest, update, channel, prerelease, published, url, hasPackage}`; `502` | `sudo -n craftbox-update --check --json` (consulta o GitHub) | **SÓ RUST** |
| POST | `/api/system/update` | — | `{ok}`; `403` (não admin); `409` (já rodando); `500` | `sudo -n systemd-run --unit=craftbox-update --collect --no-block craftbox-update --yes` (fora do cgroup do painel, que é reiniciado no fim); audit `atualizar-sistema` | **SÓ RUST** |
| GET | `/api/system/update/log` | — | `{running, log}` (última execução, até 300 linhas de `/var/log/craftbox-update.log`) | — | **SÓ RUST** |
| GET | `/api/mcp/tokens` | — | `{tokens:[{id,name,user,createdAt,lastUsedAt}]}`; `403` (não admin) | — | **SÓ RUST** |
| POST | `/api/mcp/tokens` | `{name}` | `{ok, token:"cbx_…", entry}` (o token só aparece aqui); `403` | grava `mcpTokens` (hash SHA-256) no config; audit `mcp-token-criar` | **SÓ RUST** |
| DELETE | `/api/mcp/tokens` | `?id=` | `{ok}`; `403`, `404` | tira do config; audit `mcp-token-revogar` | **SÓ RUST** |
| POST | `/mcp` | JSON-RPC 2.0 (MCP, Streamable HTTP, sem sessão, sem SSE) + `Authorization: Bearer cbx_…` | resposta JSON-RPC; `202` pra notificação; `401` + `WWW-Authenticate` sem token válido; `405` pra GET/DELETE | `initialize`, `ping`, `tools/list`, `tools/call` (20 ferramentas, cada uma vira uma chamada interna a `/api/*` com sessão do dono do token; audit com ip "… via MCP"); `lastUsedAt` no máx. 1×/min | **SÓ RUST** |
